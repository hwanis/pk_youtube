<?php
  ...
  //  echo $basename = basename($_SERVER["PHP_SELF"]);
    ...
    $list =      $obj->curdItemList($page_no, $page_list_no, $where = '', $branch, 'list');
    $list_cnt =  $obj->curdItemList($page_no, $page_list_no, $where = '', $branch, 'cnt');
 ...
 for($i=count($list_cnt);$i>-1;$i--){
    ...
    isset($list_cnt[$i][it_id])?$reg=$list_cnt[$i][it_id]:$reg=0;

    if($i>0){
         $tot_daily_price = $tot_daily_price."['".$list_cnt[$i][reg_date]."',".$reg."],";
     }else{
         $tot_daily_price = $tot_daily_price."['".$list_cnt[$i][reg_date]."',".$reg."]";
     }        
...
?>
    ...
    <script type="text/javascript">
        ...
        function openChild(url, field) {
            ...
            window.open(url, 'pop', 'status=no, scroll=yes height=500, width=900, left='+ popupX + ', top='+ popupY + ', screenX='+ popupX + ', screenY= '+ popupY);
...
-->
    ...
    <div class="col-xs-12">
        ...
        <div class="row">
            <div class="col-xs-12">
                <form name="reg_form"  action="daily_sales.php?st_date=<?=$st_date?>&ed_date=<?=$ed_date?>"  class="form-horizontal" role="form" method="post" enctype="multipart/form-data">
                    <button class="btn btn-message" type="button" name="btnSubmit" onclick="openChild('item_pop.php', this);">
                ...
                <table id="dynamic-table" class="table table-striped table-bordered table-hover">
                    <thead>
                        <tr>                         
                            ...
                            <th class="center">제품번호</th>
                            <th class="center">제품명</th>
                            <th class="center">크리에이터 ID</th>
                            <th class="center">제품구분</th>
                                               
                            <th class="center">옵션</th>
                            <th class="center">웹/cs</th>
                    ...
                    ?>
                        <tr>
                            <td class="center">
                                <button class="btn btn-xs btn-info"  onclick="openChild('item_mod_pop.php?it_id=<?=$list[$i][it_id]?>', this);">
                                    <i class="ace-icon fa fa-pencil bigger-160"></i>
                                </button>                             
                            </td>
                            <td class="right"><?=$list[$i][it_id]?></td>
                            <td class="right"><?=$list[$i][it_name]?></td>
                            <td class="right"><?=$list[$i][ct_id]?></td>

                            <td  class="center">
                                <?php
                                    switch ($list[$i][it_class]){
