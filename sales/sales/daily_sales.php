<?php
  ...
  //  echo $basename = basename($_SERVER["PHP_SELF"]);
    ...
    $where = "a.sal_date BETWEEN '$st_date' AND '$ed_date'";      
  //  $branch = $_SESSION[branch];
    switch($_SESSION[s_auth]){            
        case 0: $where = " a.sal_date BETWEEN '$st_date' AND '$ed_date' ";;break;
        case 1: $where = " a.sal_date BETWEEN '$st_date' AND '$ed_date' AND ct_id = '$_SESSION[s_id]' "; break;            
    }
   
    $list =      $obj->getDailyList($page_no, $page_list_no, $where, 'pk_tlsdmlgkstn', 'list', $_SESSION[s_auth]);    
    $list_cnt =  $obj->getDailyList($page_no, $page_list_no, $where, 'pk_tlsdmlgkstn', 'cnt', $_SESSION[s_auth]);    
        ...
        <div class="row">
            <div class="col-xs-12">
                <table id="dynamic-table" class="table table-striped table-bordered table-hover">
                    <thead>
                        <tr>
                            <th class="center" style="width:33%"><label class="pos-rel"><span class="lbl">검색날짜</span></label></th>
                            <th class="center" style="width:33%">총 판매 금액</th>
                            <th class="center" style="width:33%">총 판매 수량</th>                        
                        </tr>
                    </thead>
                    <tbody>                  
                        <tr>
                            ...
                            <td style="text-align: right"><?=number_format($bw_tot_price)?></td>
            ...
            <div class="col-xs-12" style="margin-bottom: 10px">
                ...
                ?>
                 <a href="daily_sales_exc.php?page_no=<?=$page_no?>&page_list_no=<?=$page_list_no?>&where=<?=$where?>&req_s_id=<?=$req_s_id?>&tag=cnt&s_auth=<?=$_SESSION[s_auth]?>" id="id-btn-dialog2" class="btn btn-info btn-sm">
                    엑셀 다운로드
                 </a> 
                <?php
                    }
            ...
            <div class="col-xs-12">
                <table id="dynamic-table" class="table table-striped table-bordered table-hover">
                    <thead>
                        <tr>
                            <th class="center" style="width:16%"><label class="pos-rel"><span class="lbl">주문날짜</span></label></th>
                            <th class="center" style="width:14%">판매 수량</th>      
                            <th class="center" style="width:14%">판매 금액</th>
                            <th class="center" style="width:14%">취소(반품/환불) 수량</th>
                            <th class="center" style="width:14%">취소(반품/환불) 금액</th>                            
                            <th class="center" style="width:14%">총 판매 수량</th>
                            <th class="center" style="width:14%">총 판매 금액</th>                            
                    ...
                    ?>
                        <tr>
                            ...
                            <td style="text-align: right"><?=number_format($list[$i][cn_qty])?></td>
                            <td style="text-align: right"><?=number_format($list[$i][cn_price])?></td>
                            <td style="text-align: right"><?=number_format($list[$i][d_tot_qty]-$list[$i][cn_qty])?></td>
                            <td style="text-align: right"><?=number_format($list[$i][d_tot_price]-$list[$i][cn_price])?></td>
                        </tr>
                    <?php 
                        } 
                    ?>											
        ...
        <!-- pagination -->
            <div class="center">
                <ul class="pagination">                                
                    ...
                    ?>
                        <li <?=$class_active?>>
                            <a href="<?=$basename?>?page_no=<?=$i?>&block_no=<?=$cur_block_no?>&st_date=<?=$st_date?>&ed_date=<?=$ed_date?>"><?=$i?></a>
                        </li>
                    <?php
                    }                                    
                    ...
                    <li class="disabled">                                    
                     ...
                     <?php
                        ...
                        ?>
                    <li>                                    
                         <a href="<?=$basename?>?page_no=<?=$block_cnt*$cur_block_no+1?>&block_no=<?=(int)$cur_block_no+1?>&st_date=<?=$st_date?>&ed_date=<?=$ed_date?>">
                            <i class="ace-icon fa fa-angle-double-right"></i>
                        </a>                                   
                    </li>
                        <?php } ?>
