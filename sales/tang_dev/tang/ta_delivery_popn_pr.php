...
//        require_once "../class/class_db.php";
    //require_once "../class/class_shipTa.php";
    ...
    switch($_REQUEST[tag]){
        ...
        case 'u': UpdatePyeongang($_REQUEST); break;
        case 'i': 
            $res = RegistTangOrder($_REQUEST); 
            ...
            $res = RegistPrescription($_REQUEST);
            if( gettype($res) == string){
                echo '<script type="text/javascript">                      
                        window.location = "../common/svr_error.php?msg='.$res.'/'.$basename.'";
                </script>';
            ...
            $res = RegistPreScriptionDetail($_REQUEST);
            if( gettype($res) == string){
                echo '<script type="text/javascript">                      
                    window.location = "../common/svr_error.php?msg='.$res.'/'.$basename.'";
                </script>';
   ...
   function UpdatePyeongang($req_array){
       ...
       //tdate:예약일       
        ...
        $py_array[od_number] = $_REQUEST[od_number];
        $py_array[hsid] = $_REQUEST[hsid];
        $py_array[hsid_sub] = $_REQUEST[hsid_sub];
        //$py_array[br_code] = $_REQUEST[yynumber];
        $py_array[name] = $_REQUEST[name];
        //$py_array[cs_jumin] = $_REQUEST[jumin];
        //$py_array[treat_date] = $_REQUEST[jakupdate];
        $py_array[tel] = $_REQUEST[tel_1].'-'.$_REQUEST[tel_2].'-'.substr($_REQUEST[tel_3], 0,4);         
        $py_array[handphone] = $_REQUEST[hp_1].'-'.$_REQUEST[hp_2].'-'.substr($_REQUEST[hp_3], 0,4);
        $py_array[address] = $_REQUEST[address];
       // $py_array[tname] = $_REQUEST[tname];
       // $py_array[tpaek] = $_REQUEST[tpaek];
        //$post_array[tdang] = $_REQUEST[tdang];
        $py_array[tmemo] = $_REQUEST[tmemo];  
      //  $py_array[delivery_date] = $_REQUEST[reserve_date_1];      
       // $py_array[treat_date] = $_REQUEST[jakupdate];
      //  $py_array[delivery_date] = $_REQUEST[reserve_date_1];        
        //$py_array[ta_status] = 102;
        $py_array[edtime] = $reg_date;
        //$py_array[ta_delivery] = $_REQUEST[ta_delivery];
        $py_array[register] = $_REQUEST[register];
        
        ArrayUpdate($py_array, $table = 'pyeongang', $where = 'hsid ='.$_REQUEST[hsid]);
    ...
    function RegistTangOrder($req_array){
     ...
     //   $order_array[modify_date] = $reg_date;
       ...
       $file_re_name = fileRename($_FILES["attach_file"]["name"]);  
       
       echo $_FILES["attach_file"]["name"].'<br>';
       echo $_FILES["attach_file"]["size"];
       exit;

        // finle oupload
       fileUpload($_FILES["attach_file"]["name"], $_FILES["attach_file"]["size"], $_FILES["attach_file"]["tmp_name"], $file_re_name);

       $get_post[od_number] = $_REQUEST[od_number];     
       $get_post[file_name] = $file_re_name;     
       $get_post[file_size] =  $_FILES["attach_file"]["size"];    
        $get_post[register] =  $_REQUEST[register];    
      ...
      //    echo $k.'=>'.$v.'<br>';
       ...
       // insert portfolio
       $insert = $obj->registUpExlFile($get_post);
